...
function agregarEtiquetaTicket($ticket_id, $etiqueta_id) {
    $db = Database::getInstance();
    try {
        $db->query("INSERT INTO ticket_etiquetas (ticket_id, etiqueta_id) VALUES (?, ?) ON CONFLICT DO NOTHING", 
...
function eliminarEtiquetaTicket($ticket_id, $etiqueta_id) {
    $db = Database::getInstance();
    $db->query("DELETE FROM ticket_etiquetas WHERE ticket_id = ? AND etiqueta_id = ?", 
        [$ticket_id, $etiqueta_id]);
}

function crearEtiqueta($nombre, $color = '#6c757d') {
    $db = Database::getInstance();
    return $db->insert("INSERT INTO etiquetas (nombre, color) VALUES (?, ?) RETURNING id", 
...
function crearNotificacion($usuario_id, $ticket_id, $tipo, $titulo, $mensaje = null) {
    $db = Database::getInstance();
    $db->query("
        INSERT INTO notificaciones (usuario_id, ticket_id, tipo, titulo, mensaje)
        VALUES (?, ?, ?, ?, ?)
    ", [$usuario_id, $ticket_id, $tipo, $titulo, $mensaje]);
}
...
function verificarCondiciones($ticket_id, $condiciones, $datos_extra) {
    ...
    foreach ($condiciones as $cond) {
        ...
        switch ($operador) {
            case 'igual':
                if ($valor_actual != $valor) return false;
                break;
            case 'no_igual':
                if ($valor_actual == $valor) return false;
                break;
            case 'contiene':
                if (strpos($valor_actual, $valor) === false) return false;
                break;
            case 'mayor_que':
                if ($valor_actual <= $valor) return false;
                break;
        ...
        switch ($tipo) {
            ...
            case 'notificar':
                ...
                if ($ticket && $ticket['asignado_id']) {
                    crearNotificacion($ticket['asignado_id'], $ticket_id, 'automatizacion', "Automatización ejecutada", $ticket['asunto']);
                }
                break;
        }
    }
